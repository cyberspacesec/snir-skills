...
::: warning 五个真实痛点
1. 截图容易，证据难 —— 工具零散、结果格式不一、难以关联
2. 批量与并发难 —— 并发/重试/失败隔离/流式写出要从零造轮子
3. 浏览器资源昂贵 —— 每任务拉一个 Chrome，几十并发吃满机器
4. AI 代理"不知道怎么用" —— 没有自描述技能入口，集成成本高
5. 反检测与模拟难 —— UA/指纹/IP 封禁，每项都得自己实现
:::
...
::: details 没有 snir：混乱的零散管线
```mermaid
flowchart TD
    L[shell 循环 + xargs -P] --> CS[chrome --screenshot]
    L --> CU[curl 抓 HTML]
    L --> SC[自写脚本抓 Cookie]
    CS --> D1[/截图散落目录/]
    CU --> D2[/HTML 命名不一/]
    ...
    D1 & D2 & D3 --> Q[??? 无法关联]
    Q --> Man[手写重试/限流/黑名单]
    style Q fill:#fde8e8,stroke:#d23a3a
    style Man fill:#fff4e6,stroke:#e8a317,color:#1a1a1a
```
