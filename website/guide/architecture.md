...
四种调用形态，详见 [集成模式](./integration-modes)：

- `cmd/` — cobra CLI（`scan`/`api`/`provider`/`report`/`webserve`/`version`）
- `pkg/api` — HTTP API server
- `pkg/sdk` — Go 类型化客户端（Builder 模式）
- `SKILL.md` — AI 代理自发现入口
...
- `pkg/runner` — 核心：`Driver` 接口、`ChromeDP` 实现、`DriverPool`/`PoolDriver` 池、`Runner` 执行器、`Writer`、Cookie/Proxy/Blacklist/Device/Discovery
